...
class HashMLPDensityFieldOcc(Field):
    ...
    def get_density(self, ray_samples: RaySamples, mask_to_train:Tensor = None) -> Tuple[Tensor, None]:
        ...
        if mask_to_train is not None:
            ...
            if not mask_to_train.any():
                density = torch.zeros([*batch_shape,1]).to(positions)
                return density, None
            
        if not self.use_linear:
            density_before_activation = (
                self.mlp_base(positions_flat).to(positions)
            )
        else:
        ...
        if mask_to_train is not None:
            ...
            density_before_activation = density_all.reshape(*ray_samples.frustums.shape, -1)
        else:
    ...
    def density_fn(
        self, positions: Shaped[Tensor, "*bs 3"], times: Optional[Shaped[Tensor, "*bs 1"]] = None, mask_to_train:Tensor = None
    ) -> Shaped[Tensor, "*bs 1"]:
        ...
        density, _ = self.get_density(ray_samples, mask_to_train)
        return density
