...
def grid_to_world(x_grid, voxel_size):
  ...
  def get_x():
    return (WORLD_MAX - voxel_size[0] / 2) - voxel_size * x_grid[Ellipsis, [0]]

  def get_y():
    return (WORLD_MIN + voxel_size[1] / 2) + voxel_size[1] * x_grid[Ellipsis, [2]]
  
  def get_z():
    return (WORLD_MIN + voxel_size[2] / 2) + voxel_size[2] * x_grid[Ellipsis, [1]]
...
def world_to_grid(x, voxel_size):
  ...
  def get_x():
    return ((WORLD_MAX - voxel_size[0] / 2) - x[Ellipsis, [0]]) / voxel_size

  def get_z():
    return (x[Ellipsis, [1]] - (WORLD_MIN + voxel_size[1] / 2)) / voxel_size

  def get_y():
    return (x[Ellipsis, [2]] - (WORLD_MIN + voxel_size[2] / 2)) / voxel_size
...
def calculate_grid_config(config, aabb=None):
  ...
  if aabb==None:
    return dict(
        line_voxel_size = line_voxel_size * torch.ones(3),
        triplane_voxel_size=triplane_voxel_size * torch.ones(3),
        sparse_grid_voxel_size=sparse_grid_voxel_size * torch.ones(3),
        voxel_size_to_use=voxel_size_to_use,
        resolution_to_use=resolution_to_use,
    )
  else:
    scale_z = ((aabb[1] - aabb[0]).max() / (aabb[1,2] - aabb[0,2])) // 2
    scale_z = torch.tensor([1.0,1.0,2**scale_z.item()])
    scale_z_re = torch.tensor([1.0,1.0,0.5**scale_z.item()])
    return dict(
        line_voxel_size = line_voxel_size * torch.ones(3) * scale_z,
        triplane_voxel_size=triplane_voxel_size * torch.ones(3) * scale_z,
        sparse_grid_voxel_size=sparse_grid_voxel_size * torch.ones(3) * scale_z,
        line_resolution = config.line_resolution * torch.ones(3) * scale_z_re,
        triplane_resolution=config.triplane_resolution * torch.ones(3) * scale_z_re,
        sparse_grid_resolution=config.sparse_grid_resolution * torch.ones(3) * scale_z_re,
        voxel_size_to_use=voxel_size_to_use,
        resolution_to_use=resolution_to_use,
    )
...
def get_eval_positions_and_local_coordinates(positions, cam_idx, config, grid_config):
  ...
  if config.triplane_resolution.any():
    if config.sparse_grid_resolution.any():
