...
from merf.grid_utils import (calculate_grid_config,
                             calculate_grid_config_highres,
                             get_eval_positions_and_local_coordinates,
                             interpolate_based_on_local_coordinates,
                             cal_fea_wo_interp)
from merf.grid_utils import (get_eval_positions_and_local_coordinates_inte,
                             get_eval_positions_and_local_coordinates_wointe)
...
class MERFactoField(Field):
    ...
    def evaluate_to_baking(self, positions: Tensor, appearance_embedding = None) -> Tuple[Tensor, Tensor]:
        ...
        if self.spatial_distortion is not None:
            ...
            tgpositions = SceneBox.get_normalized_positions(positions, self.aabb)
    ...
    def get_outputs(
        self, ray_samples: RaySamples, viewdirs: Optional[Tensor]=None, positions=None, samples_transform=None, appearance_embedding=None, mask_to_train:Tensor = None, use_highres:bool = False
    ) -> Dict[FieldHeadNames, Tensor]:
        
        outputs = {}
        if use_highres:
            grid_config = calculate_grid_config_highres(self.merf_model_config)
        else:
        ...
        if mask_to_train is not None:
            ...
            tgpositions = tgpositions[mask_to_train]
            if not mask_to_train.any():
                density_all = torch.zeros([*batch_shape,1]).to(positions)
                outputs.update({FieldHeadNames.DENSITY: density_all})
                features_all = torch.zeros([*batch_shape,self.geo_feat_dim]).to(positions)
                outputs.update({"features":features_all})
                return outputs

        if self.spatial_distortion is not None:
            tgpositions, triplane_positions_local, sparse_grid_positions_local, line_positions_local,  camera_indices = (
                get_eval_positions_and_local_coordinates(tgpositions, None, self.merf_model_config, grid_config)
            )
            ...
            tgpositions = SceneBox.get_normalized_positions(tgpositions, self.aabb)
        ...
        if self.use_appearance_embedding or appearance_embedding is not None:
            ...
            density = interpolate_based_on_local_coordinates(
                density, triplane_positions_local, sparse_grid_positions_local, line_positions_local, self.merf_model_config
            )  # U*Kx1.
            ...
            features = interpolate_based_on_local_coordinates(
                features, triplane_positions_local, sparse_grid_positions_local, line_positions_local, self.merf_model_config
            )  # U*Kx7.
            ...
            # todo
            # code_freq_mask = torch.ones_like(hash_features)
            # code_freq_mask[:,10:] *= 0.0
            # code_freq_mask = code_freq_mask>0.0
            # hash_features = torch.where(code_freq_mask, hash_features, 0.0)
            
            h = self.density_and_feature_network(hash_features)
            density, features = torch.split(h, [1, self.geo_feat_dim], dim=-1)
           
            
            if self.merf_model_config.virtual_grid_corner:
                density = simulate_quantization(
                    density, self.merf_model_config.range_density[0], self.merf_model_config.range_density[1]
                )
            ...
            if self.merf_model_config.virtual_grid_corner:
                ...
                density = cal_fea_wo_interp(density, self.merf_model_config)
            density = trunc_exp(density - 1.0)
            
            if mask_to_train is not None:
                density_all = torch.zeros(*batch_shape).to(positions).reshape(-1,1)
                density_all[mask_to_train] = density.reshape(-1,1)
                density = density_all.reshape(*batch_shape,-1)
            else:
                # print(density.shape)
                density = density.reshape(*batch_shape,-1)
            ...
            if self.merf_model_config.virtual_grid_corner:
                ...
                features = cal_fea_wo_interp(features, self.merf_model_config)

            features = torch.sigmoid(features)

            if mask_to_train is not None:
                features_all = torch.zeros([*batch_shape,self.geo_feat_dim]).to(positions).reshape(-1,self.geo_feat_dim)
                features_all[mask_to_train] = features.reshape(-1,self.geo_feat_dim)
                features = features_all.reshape(*batch_shape,-1)
            else:
                features = features.reshape(*batch_shape,-1)
    ...
    def forward(self, ray_samples: RaySamples, viewdirs: Optional[Tensor]=None, positions=None, mask_to_train:Tensor = None, use_highres:bool=False) -> Dict[FieldHeadNames, Tensor]:
        ...
        field_outputs = self.get_outputs(ray_samples, viewdirs=viewdirs, positions=positions, mask_to_train=mask_to_train, use_highres=use_highres)
