...
def get_train_eval_split_folder(image_filenames):
    num_images = len(image_filenames)
    # basenames = [os.path.basename(image_filename) for image_filename in image_filenames]
    basenames = [str(image_filename).split('/')[-3] for image_filename in image_filenames]
...
def get_train_eval_split_nerf(image_filenames):
    num_images = len(image_filenames)
    # basenames = [os.path.basename(image_filename) for image_filename in image_filenames]
    basenames = [str(image_filename).split('/')[-2] for image_filename in image_filenames]
    i_all = np.arange(num_images)
    i_train = []
    i_eval = []
    i_test = []
    ...
    return np.array(i_train), np.array(i_eval), np.array(i_test)
...
# @dataclass
# class MerfDataparserOutputs(DataparserOutputs):
...
class MerfDataParser(Nerfstudio):
    ...
    def _generate_dataparser_outputs(self, split="train"):
        ...
        for frame in meta["frames"]:
            ...
            #     filepath = Path(frame["file_path"]+'.png')
                # print(filepath)
        ...
        if f"{split}_filenames" in meta:
            ...
            elif self.config.eval_mode =="folder":
                i_train, i_eval = get_train_eval_split_folder(image_filenames)
                # i_eval, i_train = get_train_eval_split_folder(image_filenames)
            elif self.config.eval_mode =="nerf":
                i_train, i_eval, i_test = get_train_eval_split_nerf(image_filenames)
            ...
            if split == 'test' and self.config.eval_mode =="nerf":
               indices = i_test
        ...
        print("x_min:", poses[:,:1,3].min(), "x_max", poses[:,:1,3].max())
        print("y_min:", poses[:,1:2,3].min(), "y_max", poses[:,1:2,3].max())
        print("z_min:", poses[:,2:3,3].min(), "z_max", poses[:,2:3,3].max())
        ...
        if self.config.is_transform:
            ...
            def precompute(points, maxz, minz):
                plane_min = torch.ones(512**2) * 1.0
                plane_max = torch.ones(512**2) * (-1.0)
                
                tmp_point = torch.from_numpy(points)
                x_index = torch.clamp(torch.floor((tmp_point[...,:1] + 1.0) * 512 * 0.5 / 1.0), min=0, max=511)
                y_index = torch.clamp(torch.floor((tmp_point[...,1:2] + 1.0) * 512 * 0.5 / 1.0), min=0, max=511)
                    
                xy_index = (x_index * 512 + y_index).long().reshape(-1)
                print(xy_index)
                # plane_min[xy_index] = min(plane_min[xy_index], tmp_point[:,2])
                # plane_max[xy_index] = max(plane_max[xy_index], tmp_point[:,2])

                for i in range(tmp_point.shape[0]):
                    plane_min[xy_index[i]] = min(plane_min[xy_index[i]], tmp_point[i,2])
                    plane_max[xy_index[i]] = max(plane_max[xy_index[i]], tmp_point[i,2])

                plane_max[plane_max==-1] = torch.ones((plane_max==-1).sum()) * maxz
                plane_min[plane_min==1] = torch.ones((plane_min==1).sum()) * minz

                return plane_max, plane_min
            plane_max, plane_min = precompute(points, maxz, minz) 
            pre_plane = torch.stack((plane_min, plane_max), dim=1)
        ...
        import json
        with open(os.path.join(data_dir, 'transforms_new.json'), 'w') as f:
