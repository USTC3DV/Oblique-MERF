...
class NerfactoFieldOcc(Field):
    ...
    def get_density(self, ray_samples: RaySamples, mask_to_train:Tensor = None) -> Tuple[Tensor, Tensor]:
        ...
        if mask_to_train is not None:
            ...
            if not mask_to_train.any():
                density = torch.zeros([*batch_shape,1], dtype=torch.float16).to(positions.device)
                base_mlp_out = torch.zeros([*batch_shape,self.geo_feat_dim], dtype=torch.float16).to(positions.device)
                return density, base_mlp_out
            
        h = self.mlp_base(positions_flat)
        density_before_activation, base_mlp_out = torch.split(h, [1, self.geo_feat_dim], dim=-1)
        
        # alpha =  1.0 - trunc_exp(- density_before_activation  * 0.004)
        # density_before_activation = torch.where(alpha >= self.alpha_threshold, density_before_activation, 0.0)
        
        if mask_to_train is not None:
            density_all = torch.zeros(*batch_shape, dtype=torch.float16).to(positions.device).reshape(-1,1)
            density_all[mask_to_train] = density_before_activation.reshape(-1,1)
            density_before_activation = density_all.reshape(*batch_shape,-1)
            base_mlp_out_all = torch.zeros([*batch_shape,self.geo_feat_dim], dtype=torch.float16).to(positions.device).reshape(-1,self.geo_feat_dim)
            base_mlp_out_all[mask_to_train] = base_mlp_out.reshape(-1,self.geo_feat_dim)
            base_mlp_out = base_mlp_out_all.reshape(*ray_samples.frustums.shape, -1)
        else:
            density_before_activation = density_before_activation.view(*ray_samples.frustums.shape, -1)
            base_mlp_out = base_mlp_out.view(*ray_samples.frustums.shape, -1)
    ...
    def get_density_only(self, positions: Tensor = None, mask_to_train:Tensor = None) -> Tuple[Tensor, Tensor]:
        """Computes and returns the densities."""
        if self.spatial_distortion is not None:
            positions = self.spatial_distortion(positions)
            positions = (positions + 2.0) / 4.0
        ...
        if mask_to_train is not None:
            ...
            density_before_activation = density_before_activation.view(*batch_shape, -1)
    ...
    def get_outputs(
        self, ray_samples: RaySamples, density_embedding: Optional[Tensor] = None, mask_to_train:Tensor = None
        ...
        if mask_to_train is not None:
            rgb_all = torch.zeros([*outputs_shape, 3]).to(directions).reshape(-1,3)
            rgb_all[mask_to_train] = rgb.reshape(-1,3)
            rgb = rgb_all.reshape(*outputs_shape, -1)
        else:
    ...
    def forward(self, ray_samples: RaySamples, compute_normals: bool = False, mask_to_train:Tensor = None) -> Dict[FieldHeadNames, Tensor]:
        ...
        if compute_normals:
            with torch.enable_grad():
                density, density_embedding = self.get_density(ray_samples, mask_to_train=mask_to_train)
        else:
            density, density_embedding = self.get_density(ray_samples, mask_to_train=mask_to_train)

        field_outputs = self.get_outputs(ray_samples, density_embedding=density_embedding, mask_to_train=mask_to_train)
