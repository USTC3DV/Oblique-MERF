...
# distill
def calculate_grid_config_highres(config):
  """Computes voxel sizes from grid resolutions."""
  # `voxel_size_to_use` is for instance used to infer the step size used during
  # rendering, which should equal to the voxel size of the finest grid
  # (tri-plane or sparse grid) that is used.
  line_voxel_size = calculate_voxel_size(config.line_resolution) if config.line_resolution > 0 else 0
  triplane_voxel_size = calculate_voxel_size(config.triplane_resolution_before) if config.triplane_resolution_before > 0 else 0
  sparse_grid_voxel_size = calculate_voxel_size(config.sparse_grid_resolution_before)
  # Assuming that triplane_resolution is higher than sparse_grid_resolution
  # when a tri-planes are used.
  voxel_size_to_use = (
      triplane_voxel_size
      if config.triplane_resolution_before > 0
      else sparse_grid_voxel_size
  )
  resolution_to_use = (
      config.triplane_resolution_before
      if config.triplane_resolution_before > 0
      else config.sparse_grid_resolution_before
  )
  return dict(
      line_voxel_size = line_voxel_size,
      triplane_voxel_size=triplane_voxel_size,
...
def triplane_get_eval_posititons_wo_interp(x,voxel_size, dim):
  ...
  return grid_to_world(r, voxel_size), None

def get_eval_positions_and_local_coordinates_inte(positions, cam_idx, config, grid_config):
  if True:
    if cam_idx is not None:
      if len(cam_idx.shape) == 1 :
        cam_idx = cam_idx.unsqueeze(-1)
...
def get_eval_positions_and_local_coordinates_wointe(positions, cam_idx, config, grid_config):
  if True:
    if cam_idx is not None:
      if len(cam_idx.shape) == 1 :
        cam_idx = cam_idx.unsqueeze(-1)
    ...
    if config.triplane_resolution > 0:
      if config.sparse_grid_resolution > 0:
        ...
        # sparse_grid_positions = world_to_grid(sparse_grid_positions, grid_config['sparse_grid_voxel_size'])
        # sparse_grid_positions = torch.floor(sparse_grid_positions + 0.5)
        # sparse_grid_positions = grid_to_world(sparse_grid_positions, grid_config['sparse_grid_voxel_size'])
        if cam_idx is not None:
    ...
    if cam_idx is not None:
      cam_idx = cam_idx.reshape(-1, *cam_idx.shape[2:])
    return positions, None, None, None, cam_idx
...
def cal_fea_wo_interp(y, config):
  def calculate_num_evaluations_per_sample_wo_interp(config):
    assert (
        config.triplane_resolution > 0 or config.sparse_grid_resolution > 0
    ), 'one or both of these values needs to be specified'
    x = 0
    if config.triplane_resolution > 0:
      if config.only_xyplane:
        ...
        x += 3  # Three planes and 4 samples for bi-linear interpolation.
    if config.sparse_grid_resolution > 0:
  ...
  s = calculate_num_evaluations_per_sample_wo_interp(config)
  y = y.reshape(-1, s, *y.shape[1:])
  y = y.sum(dim=1)
  return y
